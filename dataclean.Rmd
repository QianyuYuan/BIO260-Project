...
```{r}
PA<-business_simple%>%filter(state%in%c("PA"))%>%group_by(city)%>%summarize(n=n())%>%filter(n>50)
##only Pittsburgh
NC<-business_simple%>%filter(state%in%c("NC"))%>%group_by(city)%>%summarize(n=n())%>%filter(n>50)
##only Charlotte
...
AZ<-business_simple%>%filter(state%in%c("AZ"))%>%group_by(city)%>%summarize(n=n())%>%filter(n>50)
## Phoenix Scottsdale Tempe Mesa Chandler Gilbert Glendale
NV<-business_simple%>%filter(state%in%c("NV"))%>%group_by(city)%>%summarize(n=n())%>%filter(n>50)
## Las Vegas Henderson
WI<-business_simple%>%filter(state%in%c("WI"))%>%group_by(city)%>%summarize(n=n())%>%filter(n>50)
## Madison
...
colnames(business_clean)[11] <- "Price"

## fill in the blank zip code:
fillzip<-read.csv("zipna.csv")
fillzip<-fillzip%>%select(business_id,zip)
colnames(business_clean)[3]<-"zip2"
colnames(fillzip)[2]<-"zip3"
business_clean<-left_join(business_clean,fillzip,by="business_id")
business_clean<-mutate(business_clean,zip=ifelse(is.na(zip2),zip3,zip2))%>%select(-zip2,-zip3)%>%filter(!is.na(zip)&!is.na(Price))
...
```{r}
year<-as.numeric(format(as.Date(joined$date), "%Y")) 
hist(year)
```

check the missing value
```{r}
data.frame(n.na=sapply(joined, function(x) sum(is.na(x)))) 
restaurant <- data.frame(lapply(joined, as.character), stringsAsFactors=FALSE)
